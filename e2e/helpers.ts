...
export async function seedProject(page: Page, name = "E2E Project"): Promise<string> {
  ...
  const project = {
    ...
    locale: "en",
    materials: [
      { id: "concrete", name: "Concrete", attenuationDb: { "2.4": 12, "5": 15, "6": 17 }, isDefault: true },
...
export async function seedProjectWithObjects(page: Page, name = "E2E Objects"): Promise<string> {
  ...
  const ap = {
    ...
    antennaAssignments: [],
    radios: [
      { band: "2.4", enabled: true, txPowerDbm: 15, txPowerAuto: true, channel: 0, channelAuto: true, channelWidthMHz: 40, spatialStreams: 2, antennaGainDbi: 5 },
      { band: "5", enabled: true, txPowerDbm: 15, txPowerAuto: true, channel: 0, channelAuto: true, channelWidthMHz: 40, spatialStreams: 2, antennaGainDbi: 5 },
      { band: "6", enabled: true, txPowerDbm: 15, txPowerAuto: true, channel: 0, channelAuto: true, channelWidthMHz: 80, spatialStreams: 2, antennaGainDbi: 5 },
    ],
  ...
  const project = {
    ...
    thresholds: {
      dataRssiDbm: -67, voiceRssiDbm: -65, highDensityRssiDbm: -62, minSnrDb: 25,
      minSecondaryRssiDbm: -72, maxClientsPerAp: 30, minThroughputMbps: 25,
    },
    scenarios: [
      {
        id: scnId,
        ...
        floors: [
          {
            ...
            index: 0,
            ceilingHeightM: 3,
            plan: { id: `plan_${id}`, fileName: "e2e.png", imageSrc: png, widthPx: 1200, heightPx: 800, metersPerPixel: 0.05, locked: false, opacity: 1 },
            walls: [wall],
            accessPoints: [ap],
            requirements: [],
          },
